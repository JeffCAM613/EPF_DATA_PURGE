...
# Query Oracle's own server time so comparisons against log_timestamp
# (which uses SYSTIMESTAMP) are in the same timezone.
monitor_start_utc=$(sqlplus -L -S "$CONN_STR" <<'EOSQL' 2>/dev/null | tr -d '[:space:]'
SET HEADING OFF FEEDBACK OFF PAGESIZE 0 LINESIZE 100 TRIMOUT ON TRIMSPOOL ON
SELECT TO_CHAR(SYSTIMESTAMP, 'YYYY-MM-DD HH24:MI:SS') FROM dual;
EXIT;
EOSQL
)
if [[ -z "$monitor_start_utc" || ! "$monitor_start_utc" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2} ]]; then
    monitor_start_utc=$(date -u '+%Y-%m-%d %H:%M:%S')
    write_log "[MONITOR] WARNING: Could not query server time; using client UTC."
fi
write_log "[MONITOR] Server baseline: $monitor_start_utc"
write_log ""
...
while [[ $done_flag -eq 0 ]]; do
    ...
    if [[ $found_run -eq 0 ]]; then
        run_id=$(sqlplus -L -S "$CONN_STR" <<ENDSQL 2>/dev/null | tr -d '[:space:]'
...
ENDSQL
        ...
        if [[ -n "$run_id" && ${#run_id} -ge 16 ]]; then
            ...
            # launched. Prevents replaying old DELETE / RECLAIM rows
            # when the reclaim attaches to an existing purge run_id.
            skip_id=$(sqlplus -L -S "$CONN_STR" <<EOSKIP 2>/dev/null | tr -d '[:space:]'
SET HEADING OFF FEEDBACK OFF PAGESIZE 0 LINESIZE 100 TRIMOUT ON TRIMSPOOL ON
SELECT NVL(MAX(log_id), 0) FROM oppayments.epf_purge_log
WHERE run_id = HEXTORAW('${run_id}')
  AND log_timestamp < TO_TIMESTAMP('${monitor_start_utc}', 'YYYY-MM-DD HH24:MI:SS');
EXIT;
EOSKIP
            )
            if [[ "$skip_id" =~ ^[0-9]+$ && $skip_id -gt 0 ]]; then
                last_log_id=$skip_id
                write_log "[MONITOR] Skipping $last_log_id pre-existing log entries."
            fi
    ...
    if [[ $found_run -eq 1 ]]; then
        output=$(sqlplus -L -S "$CONN_STR" <<ENDSQL 2>/dev/null
...
ENDSQL
        ...
        any_new=0
        while IFS='|' read -r log_id ts module operation batch_num rows_aff status message elapsed tbl_name error_msg; do
            ...
            error_msg=$(echo "${error_msg:-}" | xargs)
            ...
            case "$operation" in
                ...
                PHASE_START)
                    ...
                    ;;
                PHASE_END)
                    line="[$ts] <<< PHASE done: $message"
                    ;;
                SHRINK_DONE)
                    line="[$ts] $(printf '%-12s' RECLAIM) $message (${elapsed}s)"
                    ;;
                SHRINK_PROGRESS|SQUEEZE_PROGRESS|CAPTURE_DDL|LOG_RELOCATE|DROP_FK|DROP_PK|DROP_IDX|RESIZE_DF|RECREATE_IDX|RECREATE_PK|RECREATE_FK|DDL_BACKUP|GUARD|SCRATCH_TS|INTERMEDIATE|DRAIN_START|DRAIN_PROGRESS|DRAIN_DONE|REFILL_START|REFILL_PROGRESS|REFILL_DONE)
                    line="[$ts] $(printf '%-12s' RECLAIM) $message (${elapsed}s)"
                    ;;
                DELETE)
                    if [[ -n "$batch_num" ]]; then
                ...
                *)
                    if [[ "$status" == "ERROR" ]]; then
                        ...
                        if [[ -z "$display_msg" && -n "$error_msg" ]]; then
                            display_msg="$error_msg"
                        elif [[ -z "$display_msg" ]]; then
                            display_msg="(no details)"
                        fi
                        write_log "[$ts] *** ERROR *** $module - $display_msg"
                        # Only ORCHESTRATOR errors are fatal; module-level
                        # errors are non-fatal -- the orchestrator continues.
        ...
        if [[ $any_new -eq 0 && $last_log_id -gt 0 && $done_flag -eq 0 ]]; then
            latest_id=$(sqlplus -L -S "$CONN_STR" <<'ENDSQL' 2>/dev/null | tr -d '[:space:]'
SET HEADING OFF FEEDBACK OFF PAGESIZE 0 LINESIZE 300 TRIMOUT ON TRIMSPOOL ON
SELECT RAWTOHEX(run_id) FROM (
    SELECT run_id FROM oppayments.epf_purge_log
...
ENDSQL
            ...
            if [[ -n "$latest_id" && ${#latest_id} -ge 16 && "$latest_id" != "$run_id" ]]; then
                ...
                end_count=$(sqlplus -L -S "$CONN_STR" <<ENDSQL 2>/dev/null | tr -d '[:space:]'

...
DECLARE
    ...
    -- Same module classification as capture_space_snapshot in epf_purge_pkg.
    -- Kept in sync manually -- if the table lists ever drift, the prompt and
    -- the comparison report will disagree.
...
BEGIN
    ...
    BEGIN
        FOR rec IN (
            SELECT classify(sg.owner, COALESCE(l.table_name, sg.segment_name)) AS module,
            ...
            GROUP BY classify(sg.owner, COALESCE(l.table_name, sg.segment_name))
        ) LOOP
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            FOR rec IN (
                SELECT classify(USER, COALESCE(l.table_name, sg.segment_name)) AS module,
                       SUM(sg.bytes) AS bytes
                FROM user_segments sg
                LEFT JOIN user_lobs l ON l.segment_name = sg.segment_name
                GROUP BY classify(USER, COALESCE(l.table_name, sg.segment_name))
            ) LOOP
                v_total := v_total + rec.bytes;
                CASE rec.module
                    WHEN 'PAYMENTS'        THEN v_pay := v_pay + rec.bytes;

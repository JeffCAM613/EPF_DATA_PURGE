...
-- ============================================================================
-- EPF_DDL_BACKUP: persists captured DDL before indexes/constraints are dropped
-- ============================================================================
-- The reclaim script drops all indexes, PK/UK, and FK constraints during
-- tablespace compaction. If the session breaks mid-run, the in-memory DDL
-- arrays are lost and the objects cannot be recreated. This table provides
-- a durable backup. Each reclaim run writes a fresh set of rows (keyed by
-- run_id) BEFORE any DROP begins. The recovery script
-- (sql/14_recover_indexes.sql) reads from this table.
DECLARE
    l_table_exists NUMBER;
BEGIN
    SELECT COUNT(*) INTO l_table_exists
    FROM all_tables
    WHERE owner = 'OPPAYMENTS' AND table_name = 'EPF_DDL_BACKUP';

    IF l_table_exists = 0 THEN
        EXECUTE IMMEDIATE '
            CREATE TABLE oppayments.epf_ddl_backup (
                backup_id         NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                run_id            RAW(16)        NOT NULL,
                backup_timestamp  TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
                object_type       VARCHAR2(20)   NOT NULL,
                object_owner      VARCHAR2(128)  NOT NULL,
                object_name       VARCHAR2(128)  NOT NULL,
                table_name        VARCHAR2(128),
                ddl_text          CLOB           NOT NULL,
                recreated         VARCHAR2(1)    DEFAULT ''N'',
                seq_num           NUMBER,
                CONSTRAINT chk_ddl_obj_type
                    CHECK (object_type IN (''INDEX'', ''PK'', ''UK'', ''FK''))
            )';

        EXECUTE IMMEDIATE '
            CREATE INDEX oppayments.idx_epf_ddl_backup_run
            ON oppayments.epf_ddl_backup (run_id)';

        DBMS_OUTPUT.PUT_LINE('EPF_DDL_BACKUP table created successfully.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('EPF_DDL_BACKUP table already exists. Skipping creation.');
    END IF;
END;
/
